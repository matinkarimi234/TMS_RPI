...
class SessionControlWidget(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None):
        ...
        self.pause_frame = FrameButton("Pause", self)
        self.start_stop_frame = FrameButton("Start", self)

        lay = QHBoxLayout(self)
        lay.setContentsMargins(0, 0, 0, 0)
        lay.setSpacing(8)
        lay.addWidget(self.pause_frame)
        lay.addWidget(self.start_stop_frame)

        # wiring
        self.pause_frame.clicked.connect(self._on_pause_clicked)
        self.start_stop_frame.clicked.connect(self._on_start_stop_clicked)
    ...
    def set_state(self, running: bool, paused: bool):
        ...
        if running:
            self.start_stop_frame.setText("Stop")
        else:
